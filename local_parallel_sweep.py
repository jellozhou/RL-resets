...
def run_sweep(param_list, num_episodes, render_mode, evaluate_full_training):
    commands = [
        f"python one_sweep_learning_resets.py --reset_rate {params[0]} --N {params[1]} --system_size {params[2]} --learning_rate {params[3]} --gamma {params[4]} --epsilon {params[5]} --n_stable {params[6]} --reset_decay {params[7]} --num_episodes {num_episodes} --render_mode {render_mode} --resetting_mode {params[8]} --learning_end_condition {params[9]} --dimension {params[10]} --trial_num {params[11]} {'--evaluate_full_training' if evaluate_full_training else ''}"
    ...
    with ThreadPoolExecutor(max_workers=4) as executor:  # Limit the number of parallel processes
        try:
    ...
    for params in param_list:
        output_file_avg = f"results/trialnum_{params[11]}_resetrate_{params[0]}_size_{params[1]}_dimension_{params[10]}_systemsize_{params[2]}_learningrate_{params[3]}_gamma_{params[4]}_epsilon_{params[5]}_nstable_{params[6]}_learningend_{params[9]}_resetdecay_{params[7]}_numepisodes_{num_episodes}.csv"
...
# system_sizes = [2e3] # for the unbounded case
...
if __name__ == "__main__":
    evaluate_full_training = True  # Set this flag when seeking full info of the testing phase in every episode
    run_sweep(param_list, num_episodes=500, render_mode="None", evaluate_full_training=evaluate_full_training)  # when learning
