...
# standard maps, can also specify custom map
MAPS = {
...
class SimpleGridEnvResetsPBC(Env):
    """
...
# start a new episode
    ...
    def step(self, action: int):
        ...
        # Assert that action is valid (optional)
        ...
        target_row = target_row % self.nrow  # Wrap around the rows
        target_col = target_col % self.ncol  # Wrap around the columns

        # Compute the reward
        self.reward = self.get_reward(target_row, target_col)
        
        # Check if the move is valid (no need for in_bounds check anymore)
        if self.is_free(target_row, target_col):  # Assuming is_free checks if the space is not occupied
    ...
    def parse_state_option(self, state_name: str, options: dict) -> tuple:
        ...
        try:
            ...
            if isinstance(state, int): # int: is initial position
            ...
            #     return state
                ...
                raise TypeError(f'Allowed types for `{state_name}` are int, float, or tuple.')
        except KeyError:
